  ...
  const handleUpdate = async (e) => {
    ...
    try {
       ...
       if (err.code === 'auth/requires-recent-login') {
          setStatus({ type: 'error', msg: 'Sesi Anda terlalu lama. Demi keamanan, silakan Keluar (Logout) dan Masuk kembali sebelum memperbarui sandi.' });
       } else {
...
const AdminView = ({ pengampus, students }) => {
  ...
  const handleAddPengampu = async (e) => {
    ...
    try {
      ...
      setFormError(err.code === 'auth/email-already-in-use' ? "Username sudah terpakai." : "Gagal mendaftarkan akun ke sistem."); 
       ...
       for (const student of relevantStudents) {
          ...
          if (!hasRecord) {
             ...
             const payload = {
                ...
                date: yesterdayStr,
                presensi: 'Alpha',
                keterangan: 'Tercatat otomatis oleh sistem (Laporan kosong)',
                autoGenerated: true
             };
  ...
  return (
    <div className="space-y-4 md:space-y-6 pb-10">
      <div className="bg-white p-4 md:p-5 lg:p-8 rounded-2xl md:rounded-3xl shadow-sm">
         ...
         <p className="text-[10px] md:text-sm text-gray-500 mt-0.5 md:mt-1">Pantau perkembangan tahfidz dan mutaba'ah harian secara real-time.</p>
      </div>
...
const MainApp = () => {
  ...
  // Ambil data latar belakang agar bisa diverifikasi
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-800">
      {/* Header Mobile */}
      <div className="md:hidden bg-white p-4 flex items-center justify-between shadow-sm sticky top-0 z-40 border-b border-gray-100">
      ...
      <div className={`${isMobileMenuOpen ? 'block' : 'hidden'} md:block w-full md:w-64 lg:w-72 bg-white shadow-xl md:shadow-none border-r border-gray-100 flex-shrink-0 z-30 fixed md:sticky top-16 md:top-0 h-[calc(100vh-4rem)] md:h-screen overflow-y-auto`}>
        ...
        <div className="px-4 py-4 md:py-2">
           <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 mb-6 flex items-center gap-3">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-sm" style={{ backgroundColor: theme.primary }}>{user.name ? user.name.charAt(0) : 'U'}</div>
              <div className="overflow-hidden">
                 <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">{user.role}</p>
                 <p className="text-sm font-bold text-gray-800 truncate">{user.name}</p>
              </div>
           </div>

           <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-4 mb-3">Menu Utama</p>
           <nav className="space-y-1.5">
              {menuItems.map(item => {
                const Icon = item.icon;
                const isActive = activeTab === item.id;
                return (
                  <button key={item.id} onClick={() => { setActiveTab(item.id); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold text-sm ${isActive ? 'bg-gray-800 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'}`}>
      ...
      {/* Konten Menu Utama */}
      <div className="flex-1 overflow-x-hidden min-h-[calc(100vh-4rem)] md:min-h-screen relative p-4 md:p-6 lg:p-8 pb-20 md:pb-8">
      ...
      return (
        <div className="min-h-screen bg-gray-50 p-6 flex flex-col items-center justify-center font-sans">
          <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl max-w-lg w-full text-center border-t-8 border-gray-400">
            ...
            <button onClick={() => window.location.reload()} className="mt-6 px-6 py-3 bg-gray-800 text-white font-bold rounded-xl shadow-md hover:bg-gray-700 transition-colors w-full sm:w-auto">
               Muat Ulang Halaman
            </button>
            ...
            <div className="mt-6 pt-6 border-t border-gray-100">
               ...
               {this.state.showDetails && (
                  <div className="mt-4 p-4 bg-gray-900 text-green-400 text-left text-xs md:text-sm font-mono rounded-xl overflow-auto max-h-48 shadow-inner">
